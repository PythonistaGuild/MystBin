...
@limit("admin", "admin")
async def ban_user(request: Request, user_id: int, authorization: str = Depends(auth_model)) -> UJSONResponse:
    """
...
@limit("admin", "admin")
async def unban_user(request: Request, user_id: int, authorization: str = Depends(auth_model)) -> UJSONResponse:
    """
...
@limit("admin", "admin")
async def get_admin_userlist(request: Request, page: int=1, authorization: str = Depends(auth_model)):
    """
