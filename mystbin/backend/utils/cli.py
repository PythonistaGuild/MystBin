...
class CLIHandler:
    def __init__(self, app: MystbinApp) -> None:
        ...
        self.subparser_paste.add_argument("--set-password", "-sp", help="set the password for this paste", metavar=("PASTE_ID", "PASSWORD"), nargs=2)
        self.subparser_paste.add_argument("--remove-password", "-rp", help="remove the password for this paste", metavar="PASTE_ID")
    ...
    async def command_paste(self, namespace: argparse.Namespace) -> None:
        if namespace.list:
            ...
            if bad_args := find_disallowed_args(namespace, ["remove_password", "list", "set_password"]):
                print(f"Paste: The following args cannot be used with the `delete` argument: {','.join(bad_args).replace('_', '-')}")
                return
