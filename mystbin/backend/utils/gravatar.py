...
async def _gravatar_get(session: aiohttp.ClientSession, email: str) -> tuple[bool, str]:
    ...
    url = yarl.URL("https://www.gravatar.com/avatar/" + avy_hash) \
        .with_query({
            "d": "404"
        })
...
async def find_available_gravatar(emails: list[str], force_response: bool) -> str | None:
    ...
    tasks: tuple[asyncio.Task[tuple[bool, str]]] = tuple(asyncio.create_task(_gravatar_get(session, email)) for email in emails)
