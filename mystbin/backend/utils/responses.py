...
class VariableResponse(Response):
    def __init__(
        self, content: Any, request: Request, status_code: int = 200, headers: dict[str, str] | None = None
    ) -> None:
    ...
    def render(self, content: Any) -> bytes:
        if self.accept_type not in self.encoders:
            ...
            return (
                f"'{self.accept_type}' is not a supported encoding. "
                f"try one of the following: {', '.join(self.encoders.keys())}".encode()
            )  # cant have fstrings and byte strings lol
