...
class OpenAPI:
    ...
    def render_spec(self, title: str, description: str, version: str, is_public_schema: bool) -> dict[str, Any]:
        components = {}
        paths = {}
        resp = {
            "openapi": "3.0.2",
...
class ComponentProperty:
    ...
    def render(self) -> dict:
        resp: dict[str, Any] = {"title": self.title, "type": self.type}
...
class Response:
    ...
    def render(self) -> dict:
        return {
            ...
            "content": {
                self.response_type: {
                    "schema": {"$ref": f"#/components/schemas/{self.response_model.title}"} if self.response_model else {}
                }
            },
        }
...
class RouteParameter:
    ...
    def render(self) -> dict:
        return {
            ...
            "schema": {"title": self.title, "type": self.type},
        }
...
class Route:
    ...
    def render(self) -> dict:
        resp: dict[str, Any] = {
            ...
            "tags": self.tags,
            "responses": {str(status): resp.render() for status, resp in self.responses.items()},
        }
        if self.parameters:
        ...
        if self.request_body:
            resp["requestBody"] = {
                ...
                "content": {self.body_content_type: {"schema": {"$ref": f"#/components/schemas/{self.request_body.title}"}}},
            }
...
UnauthorizedResponse = Response(
    "Unauthorized",
    _Component("UnauthorizedComponent", [ComponentProperty("error", "Error", "string")], example={"error": "Unauthorized"}),
)
ForbiddenResponse = Response(
    "Forbidden",
    _Component("ForbiddenComponent", [ComponentProperty("error", "Error", "string")], example={"error": "Forbidden"}),
)
BadRequestResponse = Response(
    "Bad Request",
    _Component("BadRequestComponent", [ComponentProperty("error", "Error", "string")], example={"error": "Bad Request"}),
)
...
PasteDelete = _Component(
    "PasteDelete",
    properties=[ComponentArrayProperty("pastes", "Paste IDs", required=True, items={"type": "string"})],
    example={"pastes": ["ThreeRandomWords"]},
)

PasteDeleteResponse = Response(
    "Success",
    _Component(
        "PasteDeleteResponseComponent",
        [
...
BookmarkPutDelete = _Component(
    "BookmarkPutDelete",
    properties=[ComponentProperty("paste_id", "Paste ID", "string", required=True)],
    example={"paste_id": "ThreeRandomWords"},
)

PastePostResponse = _Component(
    "PastePostResponse",
    properties=[
...
PasteGetAllResponse = _Component(
    "PasteGetAll",
    properties=[
        ComponentArrayProperty(
            "pastes",
            "Pastes",
            True,
            items=_Component(
...
TokenListResponseTokens = ComponentArrayProperty(
    "tokens",
    "Tokens",
    True,
    _Component(
        "TokenListResponseTokens",
        [
...
LoginTokenResponse = TokenResponse = _Component(
    "LoginTokenResponse",
    properties=[
...
BookmarkResponse = _Component(
    "Bookmarks", properties=[ComponentArrayProperty("bookmarks", "Bookmarks", True, items=Bookmark)]
)
